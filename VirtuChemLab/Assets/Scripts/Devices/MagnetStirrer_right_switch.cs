...
public class MagnetStirrer_right_switch : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        magnetize = false;
        magnetismController = transform.parent.GetComponentInChildren<MagnetismController>();
    }
    ...
    void Update()
    {
        ...
        if (currentRotation > 0 && !magnetize)
        {
            magnetize = true;
            magnetismController.magnetize = true;
        }
        else if (currentRotation <= 0 && magnetize)
        {
            magnetize = false;
            magnetismController.magnetize = false;
        }
    }

    void OnMouseDrag() 
    {
        // TODO: This code might not work properly when turning the object in the scene!
