...
namespace Network
{
    ...
    public class NetworkPlayer : NetworkBehaviour
    {
        ...
        [SerializeField]
        private NetworkFakeController leftHandInteractor;
        public NetworkFakeController LeftHandInteractor => leftHandInteractor;
        
        [SerializeField]
        private NetworkFakeController rightHandInteractor;
        public NetworkFakeController RightHandInteractor => rightHandInteractor;
        
        private NetworkVariable<FixedString128Bytes> username = new NetworkVariable<FixedString128Bytes>(writePerm: NetworkVariableWritePermission.Server);

        /// <summary>
        /// The username should have a maximum length of 128 bytes!
        /// </summary>
        ...
        /// <summary>
        /// Kick this player from the session. Will
        /// only be executed on the server/host.
        /// </summary>
        public void KickPlayer()
        {
            if (IsServer)
            {
                try
                {
                    ulong clientId = NetworkLobbyManager.Singleton.GetClientIdForPlayer(this);
                    ...
                    KickPlayerClientRpc(rpcParams);
                }
                catch (ArgumentException e)
                {
                    Debug.LogWarning("Could not find player to kick");
                }
            }
        }

        /// <summary>
        /// Disconnects from the session when we receive the signal,
        /// that we have been kicked.
        /// </summary>
        [ClientRpc]
        private void KickPlayerClientRpc(ClientRpcParams rpcParams = default)
        {
            NetworkLobbyManager.Singleton.Disconnect();
        }
        ...
        /// </summary>
        /// <param username="foot">Left or right foot</param>
        /// <param username="vrHead">VR Camera position</param>
