...
public class ObjectCollisionSounds : MonoBehaviour
{
    ...
    //tells if object is inside a beaker
    [HideInInspector]
    ...
    void OnCollisionEnter(Collision coll)
    {
        ...
        if ((coll.gameObject.CompareTag("PlasticContainer") || coll.gameObject.CompareTag("PlasticObject")) && retriggerWaitTime == 0.0f && coll.relativeVelocity.magnitude > 1.1f)
        {
        ...
        if (coll.gameObject.CompareTag("GlasObject") && retriggerWaitTime == 0.0f && !isInside)
        {
        ...
        if (collMat != null && collMat.name.Contains("Material.001") && retriggerWaitTime == 0.0f && !isInside)
        {
